  ...
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="dishList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template #default="scope">
          <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['dish:dish:edit']">修改</el-button>
          <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['dish:dish:remove']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="title" v-model="open" width="500px" append-to-body>
      <el-form ref="dishRef" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="菜品单价" prop="price">
          <el-input v-model="form.price" placeholder="请输入菜品单价" />
        </el-form-item>
        <el-form-item label="图片" prop="image">
          <image-upload v-model="form.image"/>
        </el-form-item>
        <el-form-item label="描述" prop="decription">
          <el-input v-model="form.decription" placeholder="请输入描述" />
        </el-form-item>
        <el-form-item label="状态" prop="status">
        ...
        <el-table :data="dishFlavorList" :row-class-name="rowDishFlavorIndex" @selection-change="handleDishFlavorSelectionChange" ref="dishFlavor">
          ...
          <el-table-column label="口味名称" prop="name" width="150">
            <template #default="scope">
              ...
              <el-select v-model="scope.row.name" placeholder="请选择口味">
                <el-option
          ...
          <el-table-column label="口味数据" prop="value" width="150">
            <template #default="scope">
              <el-input v-model="scope.row.value" placeholder="请输入口味数据" />
            </template>
          </el-table-column>
        </el-table>
      </el-form>
...
import { listDish, getDish, delDish, addDish, updateDish } from "@/api/dish/dish";
...
function handleDeleteDishFlavor() {
  if (checkedDishFlavor.value.length == 0) {
    proxy.$modal.msgError("请先选择要删除的菜品口味数据");
  } else {
...
function handleExport() {
  proxy.download('dish/dish/export', {
    ...queryParams.value
